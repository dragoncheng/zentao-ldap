...
class ldap extends control
{
    ...
    {
        echo $this->ldap->identify($this->config->ldap->host, $this->config->ldap->dn, $this->config->ldap->pwd);
    }

    public function sync()
    {  
      if ( strstr($this->config->ldap->baseDN,'midea') )
      { //for midea: forbion to sync
        echo 'for Midea: no sync';
      }
      else
      {
    ...
    public function syncUser($user, $pwd)
    {
        $account = $this->config->ldap->uid.'='.$user.','.$this->config->ldap->baseDN;
        $users = $this->ldap->syncUser($this->config->ldap, $account, $pwd, $user);
        echo $users;
    }
}
